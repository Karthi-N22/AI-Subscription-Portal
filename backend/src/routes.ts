...
import { invoices, projects, seats, seedCredentials, subscriptions, users } from './data/store.js'
...
router.get('/invoices', (request, response) => {
  ...
  const data = monthlySubscriptions.map((subscription) => {
    const existing = invoices.find((invoice) => invoice.subscriptionId === subscription.id)
    const base = existing ?? { id: `auto-${subscription.id}-${month}`, invoiceNumber: `AUTO-${month}-${subscription.id}`, billingPeriod: monthLabel(month), fileName: `${slugify(subscription.serviceName)}-${month}.pdf`, filePath: '', amount: subscription.usdMonthlyCost, invoiceDate: subscription.renewalDate, subscriptionId: subscription.id }
    return { ...base, serviceName: subscription.serviceName, project: projectName(subscription.projectId), autoGenerated: !existing }
  })
  return response.json({ data })
})
router.post('/invoices', (request, response) => { const invoice = { id: id(), ...request.body }; invoices.push(invoice); return response.status(201).json({ data: invoice }) })
