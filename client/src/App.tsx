...
export default function App() {
  ...
  async function saveSubscription(input: Record<string, unknown>, invoiceFile?: File) {
    try {
      const selectedUserIds = Array.isArray(input.selectedUserIds) ? input.selectedUserIds.map(String) : []
      const response = await createSubscription({ ...input, selectedUserIds: undefined, month })
      await Promise.all(users.filter((user) => selectedUserIds.includes(user.id)).map((user) => createSeat({ name: user.name, email: user.email, subscriptionId: response.data.id, licenseType: 'INDIVIDUAL' })))
      let invoiceError = ''
      ...
      notify(invoiceError ? `Subscription saved — invoice upload failed: ${invoiceError}` : 'Subscription and users saved')
...
function SubscriptionModal({ projects, users, seats, month, close, save }: { projects: Project[]; users: WorkspaceUser[]; seats: Seat[]; month: string; close: () => void; save: (input: Record<string, unknown>, invoiceFile?: File) => Promise<void> }) {
