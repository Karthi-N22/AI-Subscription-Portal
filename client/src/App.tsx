...
export default function App() {
  ...
  async function loadData(selectedMonth = month) { try { setLoading(true); setError(''); const [subscriptionResponse, dashboardResponse, seatResponse, invoiceResponse, budgetResponse, userResponse, projectResponse] = await Promise.all([listSubscriptions(selectedMonth), getDashboard(selectedMonth), listSeats(), listInvoices(selectedMonth), listBudgets(selectedMonth), listUsers(), listProjects()]); setItems(subscriptionResponse.data); setDashboard(dashboardResponse.data); setScreenData({ seats: seatResponse.data, invoices: invoiceResponse.data, budgets: budgetResponse.data, users: userResponse.data }); setProjects(projectResponse.data) } catch { setError('The API is unavailable. Start the backend on port 4000 and try again.') } finally { setLoading(false) } }
  useEffect(() => { void loadData(month) }, [month])
  async function saveSubscription(input: Record<string, unknown>, invoiceFile?: File) {
    try {
      const assignedUserName = typeof input.assignedUserName === 'string' ? input.assignedUserName.trim() : ''
      const assignedUserEmail = typeof input.assignedUserEmail === 'string' ? input.assignedUserEmail.trim() : ''
      const licenseType = input.licenseType === 'TEAM' ? 'TEAM' : 'INDIVIDUAL'
      const response = await createSubscription({ ...input, assignedUserName: undefined, assignedUserEmail: undefined, licenseType: undefined, month })
      if (assignedUserName) await createSeat({ name: assignedUserName, email: assignedUserEmail, subscriptionId: response.data.id, licenseType })
      let invoiceError = ''
      if (invoiceFile) { try { await uploadInvoice(response.data.id, month, invoiceFile) } catch (error) { invoiceError = error instanceof Error ? error.message : 'Unable to upload invoice' } }
      setModal(null)
      await loadData(month)
      notify(invoiceError ? `Subscription saved — invoice upload failed: ${invoiceError}` : assignedUserName ? 'Subscription and seat saved' : 'Subscription saved')
    } catch { notify('Unable to save subscription') }
  }
  ...
  async function uploadSubscriptionInvoice(subscriptionId: string, file: File) { try { await uploadInvoice(subscriptionId, month, file); await loadData(month); notify('Invoice uploaded') } catch (error) { notify(error instanceof Error ? error.message : 'Unable to upload invoice') } }
